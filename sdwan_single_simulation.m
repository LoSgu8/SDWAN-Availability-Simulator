function [sdwan_mean_availability, sdwan_status] = ...
sdwan_single_simulation(simulation_time, sampling_time_interval, ...
tunnel_parameters, controller_parameters, controller2cpe_parameters, ...
cpe_parameters, initial_active_tunnel, single_run)
    ...
    controller_hw_signals = false(controller_parameters.nb_replicas, numel(time_points));
    controller_sw_signals = false(controller_parameters.nb_replicas, numel(time_points));
    controller_status_signals = false(controller_parameters.nb_replicas, numel(time_points));
    
    for replica=1:controller_parameters.nb_replicas
        % The hardware is completely independent from the software
        controller_hw_signals(replica,:) = compute_status_signal( ...
        ...
        % Initialize the first element
        controller_sw_signals(replica,1) = controller_parameters.sw.initial_status;
        ...
        for i = 1:length(start_idx)
            ...
            controller_sw_signals(replica, start_idx(i):end_idx(i) ) = ...
                compute_status_signal( ...
                    initial_status, ...
        ...
        % Compute the CPE status as product of hw and sw modules
        controller_status_signals(replica,:) = controller_hw_signals(replica,:) & controller_sw_signals(replica,:);
    end
    ...
    if single_run
        ...
        figure
        sgtitle('CONTROLLER')
        if controller_parameters.nb_replicas == 1
            % HW
            subplot(2 + controller_parameters.nb_replicas, 1, 1)
            ...
            grid on
            % SW
            subplot(2 + controller_parameters.nb_replicas, 1, 2)
        ...
        % CONTROLLERs
        for replica=1:controller_parameters.nb_replicas
            subplot((controller_parameters.nb_replicas==1) * 2 + controller_parameters.nb_replicas, 1, (controller_parameters.nb_replicas==1)*2 + replica)
            plot(time_points, controller_status_signals(replica,:), 'r', 'LineWidth', 2);
    ...
    sdwan_status = compute_sdwan_signal( ...
        ...
        controller_status_signals, c2cpe_signals, time_points, single_run);
    
    sdwan_mean_availability = compute_availability_from_signal(sdwan_status);
    
    if single_run
        disp(['The computed SDWAN availability is ' num2str(sdwan_mean_availability)])
    end
end